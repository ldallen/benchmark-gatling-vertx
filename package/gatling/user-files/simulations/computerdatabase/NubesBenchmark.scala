...
class NubesBenchmark extends Simulation {


	//config http générée par Gatling pour la connexion au serveur
	val httpProtocol = http
		.baseURLs("http://192.168.7.119:8090", "http://192.168.7.120:8090", "http://192.168.7.121:8090", "http://192.168.7.122:8090", "http://192.168.7.123:8090") //Adresses virtuelles pour monter à plus de 60k users
		.proxy(Proxy("192.168.2.109", 8090).httpsPort(8090))
	...
	// Valeurs d'actions utilisées pour les requetes GET localhost:8090/api/todo/action/...
	  ...
	  override def next: Map[String, String] = {
	    ...
	    val idUpdateTask = randInt(1,100)
	    ...
	    Map("actionPostTask" -> actionPostTask.toString,
	        ...
	        "idUpdateTask" -> idUpdateTask.toString,
	...
	// On utilise rampUser pour une montée en charge progressive du nombre d'utilisateurs 
	setUp(
		nubes.inject(rampUsers(100000) over (20 seconds)).throttle(reachRps(2500) in (20 second), holdFor(10 minutes))).protocols(httpProtocol)
}